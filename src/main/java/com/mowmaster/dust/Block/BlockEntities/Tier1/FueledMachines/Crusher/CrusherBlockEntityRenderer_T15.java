...
public class CrusherBlockEntityRenderer_T15 extends DustFueledMachineBlockEntityRenderer implements BlockEntityRenderer<CrusherBlockEntity_T15>
{
    public CrusherBlockEntityRenderer_T15(BlockEntityRendererProvider.Context context) {}

    @Override
    public void render(CrusherBlockEntity_T15 p_112307_, float p_112308_, PoseStack p_112309_, MultiBufferSource p_112310_, int p_112311_, int p_112312_) {
            ...
            if(!(stacksList.size() == 0))
            {
                if(facing == Direction.SOUTH) {
                    for(int i=0;i<stacksList.size();i++)
                    {
                        ...
                        if(floatValues.size()>0)renderItemRepaired(p_112307_,i,world,p_112309_,p_112310_,p_112307_.getRenderItem(stacksList.get(i)),p_112311_,p_112312_, floatValues.get(0), floatValues.get(1), floatValues.get(2), floatValues.get(3), floatValues.get(4), floatValues.get(5),floatValues.get(6),2, p_112307_.getRenderAsBlock(stacksList.get(i)));
                    }
                    if(!inputStack.isEmpty())renderItemStacked(p_112307_,world,p_112309_,p_112310_,inputStack,p_112311_,p_112312_, 0.5f, (inputStack.getItem() instanceof BlockItem)?(0.65f):(0.65f), 0.5f, 0.25f, 0.25f, 0.25f,(inputStack.getItem() instanceof BlockItem)?(0.0f):(270.0f), (inputStack.getItem() instanceof BlockItem)?(true):(false));
    ...
    public static void renderItemRepaired(DustFueledMachineBaseBlockEntity tile, int count, Level worldIn, PoseStack p_112309_, MultiBufferSource p_112310_, ItemStack itemStack, int p_112311_, int p_112312_, float x, float y, float z, float scaleX, float scaleY, float scaleZ, float angle, int axis, boolean renderAsBlock)
    {
        if (!itemStack.isEmpty()) {
            ...
            if(renderAsBlock)
            {
                ...
                if(!(blocky instanceof CampfireBlock) && tile.getBurnTime()>0)
                {
                    if(!itemStack.getItem().equals(Items.OBSIDIAN))p_112309_.mulPose(Vector3f.YP.rotationDegrees(180));
                    long time = System.currentTimeMillis();
                    int rotFast = (tile.getInputItemInSlot(1).isEmpty())?(120):(20);
                    float angleRot = (time/rotFast) % 360;
                    //float angle = (worldIn.getGameTime()) / 20.0F * (180F / (float) Math.PI);
                    p_112309_.mulPose(Vector3f.ZP.rotationDegrees(angleRot));
                }
