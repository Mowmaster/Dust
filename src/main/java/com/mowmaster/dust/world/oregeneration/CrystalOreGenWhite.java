...
public class CrystalOreGenWhite implements IWorldGenerator
{
    @Override
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
    {
        switch (world.provider.getDimensionId())
        {
            ...
            case 0: //Overworld
                this.runGenerator(this.gen_BlockOreWhite, world, random, chunkX, chunkZ, 32,1,256);// should be rare enough ;)
                break;
    ...
    public CrystalOreGenWhite()
    {
        this.gen_BlockOreWhite = new WorldGenMinable(ModBlocks.WhiteCrystalOre.getDefaultState(), 16);
    }
    ...
    private void runGenerator(WorldGenerator generator, World world, Random rand, int chunk_X, int chunk_Z, int chanceToSpawn, int minHeight, int maxHeight)
    {
            ...
            for (int i = 0; i < chanceToSpawn; i++)
            {
                ...
                if (world.getBiomeGenForCoords(new BlockPos(x, y, z)).equals(biomeCrystal.getBiome(configFile.BIOMECRYSTALWHITE))) {
